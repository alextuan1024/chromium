...
#if BUILDFLAG(IS_MAC)
void BrowserWidget::SetPageThemeColor(std::optional<SkColor> color) {
  BrowserWindowInterface* browser = browser_view_->browser();
  if (browser->GetType() != BrowserWindowInterface::Type::TYPE_NORMAL ||
      IsIncognitoBrowser()) {
    return;
  ...
  if (!color) {
    page_theme_pack_.reset();
    UserChangedTheme(BrowserThemeChangeType::kWebAppTheme);
    return;
  }

  page_theme_pack_ = base::MakeRefCounted<BrowserThemePack>(
      ui::ColorProviderKey::ThemeInitializerSupplier::ThemeType::kAutogenerated);
